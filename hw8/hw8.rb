class Triangle
	...
	def perimeter
		if(!self.test) then puts "error: not a triangle"
		else (@a+@b+@c)/2 end
	end
...
class Array
	#function checks if a[i] is in between amin and a max
	def limited?(amin, amax)
		self.each do |i|
			if i < amin || i > amax then return false
			else return true
			end
		end
	end
	#i know sorting an array is probably the most inefficient way of
	#	doing things but this really looks pretty	
	def sorted?
		ary = self.dup
		original = self.dup
		ary.sort!
